...
##     only)
##
## TW also modified the 'sig' test for significance to test for confidence
...
plotRadCon <- function(df                   # dataframe with S1 and ST indices
                       ...
                       ,legend_egg=FALSE    # plot modified legend
                       ){
...
text(-3.3, -3.0,'First-order sensitivity', cex=0.9)
arrows(x0=-2.7, y0=-3.15, x1=-2.05, y1=yloc[1]+.25, length=0.04, code=2, lwd=1.5)
text(-3.6, -3.7,'Total sensitivity', cex=0.9)
arrows(x0=-2.7, y0=-3.7, x1=-2.35, y1=-3.65, length=0.04, code=2, lwd=1.5)
...
text(0, -2.7,'First-order and total\nsensitivity magnitudes', cex=0.9)
